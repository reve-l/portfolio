...
import coolrevel from "./assets/images/cool.png";
  ...
  //://////// CONNEXION USER
    const handleFormSubmit = () => {  
      ...
      const checkEmail = (serverUsers,formData) => {
       // const user = Array.isArray(serverUsers) 
        const user = serverUsers.find(({ email }) => email === formData.email); // extract the email from the formData
         if (user) return user;
      };


      const checkPass = (serverPass,formData) => {
        // const user = Array.isArray(serverUsers) 
         const pass = serverPass.find(({ password }) => password === formData.password); // extract the email from the formData
          if (pass) return pass;
       };

      //console.log("FORM DATA ID",formData.id)    
      //fetch(url2+formData.username)
     // fetch(url2+`${formData.email}`)
      //fetch(url2+`${formData.email}`, {method:"GET",headers:{'content-type':"application/json"}} )
      ...
      .then((res) => {
        const use= checkEmail(res,formData)
        if(use.length===0){
          ...
          const pas=checkPass(res,formData)
          if(pas.length!==0){
          ...
          }else {setErrorConnect(true);          console.log('NON TROUVE !!!!!!!!!!!!!!!')
  ...
  return (
    <div className="">
              ...
              <div className="container">
                  ...
                  <section className="row ctact-me pt-5">
                    ...
                    <article className="col ctact-me-g">
                      ...
                      <form className="ctact-me-form" onSubmit={()=>handleMessage(formDatasms)}>
                        ...
                        <div className='text-center'>
                          ...
                          <div>
                            <button type="submit" className={user?"send-ctact-btn":"not-send-ctact-btn"} data-title={user?"Envoyer message":"Connectez-vous d'abord"} disabled={user?false:true} >Envoyer</button>
                          </div>
                        </div>            
                      </form>
                    ...
                    <div className='container'>
                      <div className="row mb-5 r-title">
                            ...
                            <article className=" moi-txt">
                                ...
                                <p>
                                  Nous nous appellons OKONG REVEL DIDIER (<span className='fw-bold'>ORDI</span> <img src={coolrevel} alt="ordi cool" className="img-fluid cool-r"/>)            
 passionné passionné mais alors passionné de l'Informatique, des Technologies globalement, et surtout surtout du Développement des Logiciels.
